...
(function () {
  ...
  function findAddBtn() {
    ...
    for (var i = 0; i < b.length; i++) {
      ...
      if (t.indexOf('ক্রয় তালিকায়') !== -1 || t.indexOf('ক্রয় তালিকা') !== -1) return b[i];
    }
    return null;
  }
  ...
  function render(addBtn) {
    ...
    // Try to insert before price block; fall back above add-to-cart row
    var placed = false;
    ...
    if (!placed) {
      var blk = addBtn;
      for (var u = 0; u < 4 && blk.parentElement && blk.offsetWidth < 200; u++) blk = blk.parentElement;
