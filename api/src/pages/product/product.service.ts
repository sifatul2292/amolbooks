...
<body>
  ...
  async getCustomersAlsoBought(productSlug: string): Promise<ResponsePayload> {
    try {
      ...
      const currentProductId = new ObjectId((currentProduct as any)._id);
      ...
      const productSelect =
        '_id name slug images salePrice afterDiscountPrice discountAmount discountType quantity author ratingCount ratingTotal';
      const coPurchasedProducts = rankedIds.length
        ? await this.productModel
            .find({
              _id: { $in: rankedIds.map((id: string) => new ObjectId(id)) },
              status: 'publish',
              quantity: { $gt: 0 },
            })
            ...
            .lean()
        : [];
      const coPurchasedMap = new Map(
        coPurchasedProducts.map((product: any) => [String(product._id), product]),
      );
      const orderedProducts = rankedIds
        .map((id: string) => coPurchasedMap.get(id))
        .filter(Boolean)
        .slice(0, 8);
  ...
  async findAllPublished(): Promise<Product[]> {
    return this.productModel
      .find({ status: 'publish' })
      .select(
        'slug name updatedAt category subCategory author publisher totalSold priority',
      )
  ...
  async findPublishedForSeoLanding(terms: string[], limit = 24): Promise<Product[]> {
    ...
    return this.productModel
      .find({ status: 'publish', quantity: { $gt: 0 }, ...search })
      .select(
        'name nameEn slug images salePrice afterDiscountPrice discountAmount discountType quantity author category publisher shortDescription description seoTitle seoDescription isbn sku totalSold updatedAt',
      )
      .sort({ totalSold: -1, priority: -1, updatedAt: -1 })
      .limit(limit)
      .lean()
