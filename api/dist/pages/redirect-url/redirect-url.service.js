...
var RedirectUrlService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.RedirectUrlService = void 0;
...
let RedirectUrlService = RedirectUrlService_1 = class RedirectUrlService {
    constructor(redirectUrlModel, userModel, configService, utilsService) {
        this.redirectUrlModel = redirectUrlModel;
        this.userModel = userModel;
        this.configService = configService;
        this.utilsService = utilsService;
        this.logger = new common_1.Logger(RedirectUrlService_1.name);
    }
    async addRedirectUrl(addRedirectUrlDto) {
        ...
        const mData = Object.assign(Object.assign({}, addRedirectUrlDto), defaultData);
    ...
    async insertManyRedirectUrl(addRedirectUrlsDto, optionRedirectUrlDto) {
    ...
    async getAllRedirectUrls(filterRedirectUrlDto, searchQuery) {
        ...
        try {
            const dataAggregates = await this.redirectUrlModel.aggregate(aggregateSredirectUrles);
            if (pagination) {
    ...
    async updateRedirectUrlById(id, updateRedirectUrlDto) {
        ...
        try {
            const finalData = Object.assign({}, updateRedirectUrlDto);
    ...
    async updateMultipleRedirectUrlById(ids, updateRedirectUrlDto) {
        const mIds = ids.map((m) => new ObjectId(m));
        try {
            await this.redirectUrlModel.updateMany({ _id: { $in: mIds } }, { $set: updateRedirectUrlDto });
            return {
                success: true,
                message: 'Success',
            };
    ...
    async deleteMultipleRedirectUrlById(ids, checkUsage) {
        try {
            const mIds = ids.map((m) => new ObjectId(m));
...
RedirectUrlService = RedirectUrlService_1 = __decorate([
...
exports.RedirectUrlService = RedirectUrlService;
//# sourceMappingURL=redirect-url.service.js.map