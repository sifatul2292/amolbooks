...
var ReaderClassService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.ReaderClassService = void 0;
...
let ReaderClassService = ReaderClassService_1 = class ReaderClassService {
    constructor(readerClassModel, productModel, configService, utilsService) {
        this.readerClassModel = readerClassModel;
        this.productModel = productModel;
        this.configService = configService;
        this.utilsService = utilsService;
        this.logger = new common_1.Logger(ReaderClassService_1.name);
    }
    async addReaderClass(addReaderClassDto) {
        ...
        const mData = Object.assign(Object.assign({}, addReaderClassDto), defaultData);
    ...
    async insertManyReaderClass(addReaderClasssDto, optionReaderClassDto) {
    ...
    async getAllReaderClasss(filterReaderClassDto, searchQuery) {
    ...
    async updateReaderClassById(id, updateReaderClassDto) {
        ...
        try {
            const finalData = Object.assign({}, updateReaderClassDto);
            if (name)
    ...
    async updateMultipleReaderClassById(ids, updateReaderClassDto) {
        const mIds = ids.map((m) => new ObjectId(m));
        if (updateReaderClassDto.slug) {
            delete updateReaderClassDto.slug;
        }
        try {
            await this.readerClassModel.updateMany({ _id: { $in: mIds } }, { $set: updateReaderClassDto });
            return {
                success: true,
                message: 'Success',
            };
    ...
    async deleteReaderClassById(id, checkUsage) {
        ...
        try {
            ...
            if (checkUsage) {
                ...
                await this.productModel.updateMany({ 'readerClass._id': new ObjectId(id) }, { $set: resetReaderClass });
            }
            return {
                success: true,
                message: 'Success',
    ...
    async deleteMultipleReaderClassById(ids, checkUsage) {
        try {
            const mIds = ids.map((m) => new ObjectId(m));
            ...
            if (checkUsage) {
                ...
                await this.productModel.updateMany({ 'readerClass._id': { $in: mIds } }, { $set: resetReaderClass });
            }
            return {
                success: true,
                message: 'Success',
...
ReaderClassService = ReaderClassService_1 = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, mongoose_1.InjectModel)('ReaderClass')),
...
exports.ReaderClassService = ReaderClassService;
//# sourceMappingURL=reader-class.service.js.map